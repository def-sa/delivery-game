...
@export var fixed_start_point = false
@export var fixed_end_point = false
...
func init():
	...
	for i in number_of_segments:
		...
		physics_material.friction = 1.0 # not needed?
...
func _physics_process(_delta: float) -> void:
	...
	for p in (curve.point_count):
		if  p < (number_of_segments):
			...
			var new_pos = segments[p].position + segments[p].transform.basis.y * segments[p].get_child(0).shape.height/2
			curve.set_point_position(p, new_pos)
			
		else:
			# get the last segment and add it's basis * distance to point at the endpoint 
			var new_pos = segments[p-1].position - segments[p-1].transform.basis.y * segments[p-1].get_child(0).shape.height/2
			curve.set_point_position(p, new_pos)
	
	
		if lines.is_empty():
			for i in range(curve.get_point_count() - 1):
				var from_point = curve.get_point_position(i)
				var to_point = curve.get_point_position(i + 1)
				var line = create_line(from_point, to_point)
				lines.push_back(line)
			
		for line in lines:
			if !get_children().has(line):
				add_child(line)
			else:
				line.queue_free()
				lines.erase(line)
