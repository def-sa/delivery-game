...
@export var house_size := Vector3i(4,3,4)
...
func _create_chunk(position: Vector3, index):
	...
	if index != 0:
		...
		if position.x > 0: #add structures to the rightmost chunk
			...
			_create_structure(chunk, house_structure_chance)
		if position.x < 0: #add structures to the leftmost chunk
			...
			if current_block_index == block_size - 1:
				#shop
				_create_structure(chunk, shop_structure_chance)

func _create_structure(chunk, chance):
	...
	for i in range(0, int(randf() * 10)):
		if randf() < chance:
			...
			if initial == false:
				...
				dungeon_generator.visualize_generation_wait_between_iterations = 500
			else:
				dungeon_generator.visualize_generation_wait_between_iterations = 50
			dungeon_generator.astar_heuristic = "NONE_DIJKSTRAS"
