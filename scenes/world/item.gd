class_name physicsitem extends Node3D
...
const HEALTHBAR = preload("res://scenes/ui/healthbar.tscn")
## TODO : fix rope
func _ready() -> void:
...
func _is_breakable():
	if not modifiers.has("BREAKABLE"):
		return
	
	if not health_bar_node:
		health_bar_node = HEALTHBAR.duplicate().instantiate()
		add_child(health_bar_node)
		health_bar_node.progress_bar.max_value = box_max_health
		health_bar_node.progress_bar.value = box_max_health
	else:
		health_bar_node.sprite_3d.global_position = current_box.global_position + Vector3(0,box_size.y/2,0)

# TODO: FIX 
#func _is_detected(delta):
	#var item_detection_overlay
	#
	#if not modifiers.has("DETECTABLE"):
		#return
	#
	##_clear_detected_overlays()
	#if overlays_active.has(current_box) and gui.item_detection.objects_inside_area.has(current_box):
		## Reuse the existing overlay
		#item_detection_overlay = overlays_active[current_box]
	#else:
		#
		## Create and store a new overlay
		#item_detection_overlay = ITEM_DETECTION_RECT.duplicate().instantiate()
		#gui.item_detection.add_child(item_detection_overlay)
		#overlays_active[current_box] = item_detection_overlay
		#
	#print(overlays_active)
...
func _is_scanned():
	if is_scanned and modifiers.has("SCANNABLE"):
		pass
